# hud.py: interface do jogo (HUD) e telas de início/fim
...
from config import (
    ...
    COR_HUD_BG, COR_HUD_FRACO, COR_HUD_DESTAQUE,
    COR_COMBO, COR_INIMIGO, COR_ZONA_BORDA,
    COR_VITORIA, COR_DERROTA, COR_PRETO,
    DURACAO_POWERUP
)
...
class HUD:
    ...
    def _desenhar_barra_superior(self, tela, ej):
        ...
        fonte_cmb  = pygame.font.SysFont("ocraextended", tam_fonte, bold=True)
...
def desenhar_tela_inicial(tela):
    """Tela de instruções exibida antes da partida (controls.png), aguardando ENTER."""
    global _controls_img

    # Carrega controls.png uma única vez
    ...
    if _controls_img:
        ...
        return   # A própria arte já mostra a instrução de ENTER

    # Sem a imagem: fallback com aviso pulsante de ENTER
    tela.fill(COR_PRETO)
...
def desenhar_tela_fim(tela, fonte_grande, fonte_media, fonte_pequena,
    ...
    # Título do resultado
    fonte_titulo  = pygame.font.SysFont("ocraextended", 28, bold=True)
    ...
    fonte_sub   = pygame.font.SysFont("ocraextended", 20)
    ...
    fonte_valor  = pygame.font.SysFont("ocraextended", 13, bold=True)
    fonte_rotulo = pygame.font.SysFont("ocraextended", 11)
    ...
    fonte_inst = pygame.font.SysFont("ocraextended", 12, bold=True)
