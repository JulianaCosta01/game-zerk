...
class EstadoJogo:
    ...
    def __init__(self):
        ...
        # Portas automáticas do laboratório, fixas no mapa (posições 'D' no LAYOUT_MAPA)
        self.portas = [
    ...
    def nova_partida(self):
        ...
        A cada sessão a velocidade e o alcance dos inimigos aumentam
        (até seus respectivos máximos), tornando o jogo progressivamente
        mais difícil.
        """
        self.numero_sessao += 1

        # Calcula velocidade e alcance dos inimigos desta sessão
        vel_inimigos = min(
            VELOCIDADE_INIMIGO_BASE + (self.numero_sessao - 1) * VELOCIDADE_INIMIGO_PASSO,
            VELOCIDADE_INIMIGO_MAX
        )
        alcance = min(
            INIMIGO_ALCANCE_BASE + (self.numero_sessao - 1) * INIMIGO_ALCANCE_PASSO,
            INIMIGO_ALCANCE_MAX
        )
    ...
    def atualizar(self, dt, teclas, pos_mouse, eventos):
        ...
        # Portas abrem quando o jogador ou algum inimigo vivo está próximo
    ...
    def _checar_colisoes_projetil_inimigo(self):
        ...
        (O(n × m), suficiente para o tamanho deste jogo).

        Ao acertar: desativa o projétil e o inimigo, atualiza pontuação
        e combo, e gera partículas de explosão.
        """
        for proj in self.projeteis:
            if not proj.ativo:
                continue
    ...
    def desenhar(self, tela):
        """
        Renderiza o jogo em camadas, de trás para frente: labirinto,
        itens, inimigos, projéteis, partículas, jogador, zona vermelha
        e por fim o HUD (e o overlay de fim de jogo, se aplicável).
        """
